...
.ow-blade {
  ...
  width: calc(1.5px * var(--k));
  height: calc(6px * var(--k));
  margin-left: calc(-0.75px * var(--k));
  margin-top: calc(-3px * var(--k));
  background: #fff;
  box-shadow: 0 0 0 1px rgba(0,0,0,.7), 0 0 calc(3px * var(--k)) rgba(0,0,0,.9);
  transform-origin: 50% 50%;
  will-change: transform, opacity;
}
.ow-dot {
  position:absolute; left:0; top:0;
  width: calc(2px * var(--k)); height: calc(2px * var(--k));
  margin-left: calc(-1px * var(--k)); margin-top: calc(-1px * var(--k));
  background:#fff; border-radius:50%;
  box-shadow: 0 0 0 1px rgba(0,0,0,.7), 0 0 calc(4px * var(--k)) rgba(0,0,0,.9);
...
export function installStyles() {
  // Always remove + reinstall so HMR changes take effect immediately
  document.getElementById('ow-ui-style')?.remove();
  const s = document.createElement('style');
  s.id = 'ow-ui-style';
  s.textContent = CSS;
  document.head.appendChild(s);
  const existing = document.getElementById('ow-ui-defs');
  if (!existing) {
